...
CREATE TABLE swint.tb_micro_info (
                                     ...
                                     micro_service_domain varchar NULL, -- 마이크로서비스도메인
                                     mod_timestamp timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, -- 변경일시
                                     reg_timestamp timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, -- 등록일시
...
COMMENT ON COLUMN swint.tb_micro_info.micro_service_domain IS '마이크로서비스도메인';
...
CREATE TABLE swint.tb_container_info (
                                         ...
                                         container_domain varchar NOT NULL, -- 이미지도메인
                                         mod_timestamp timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, -- 변경일시
                                         reg_timestamp timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, -- 등록일시
                                         CONSTRAINT tb_container_info_pk PRIMARY KEY (container, container_domain)
);
COMMENT ON TABLE swint.tb_container_info IS '기업의 컨테이너 정보를 저장';

-- Column comments

COMMENT ON COLUMN swint.tb_container_info.image IS '이미지ID';
COMMENT ON COLUMN swint.tb_container_info.container IS '컨테이너ID';
COMMENT ON COLUMN swint.tb_container_info.container_domain IS '이미지도메인';
